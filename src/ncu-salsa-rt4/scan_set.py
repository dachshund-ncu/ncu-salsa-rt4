...
class ScanSet:
    ...
    def make_initial_settings(self):
        ...
        self.source_J2000 = SkyCoord(ra=self.scans[0].RA * u.hourangle, dec=self.scans[0].DEC * u.degree, frame=FK5,
                                     equinox='J2000')
    ...
    def save_to_file(self, flename="WYNIK.DAT"):
        ...
        for i in range(len(self.scans)):  # i to skany (zazwyczaj 0 - 19)
            for ee in range(len(self.scans[i].auto)):  # ee to BBC (zazwyczaj 0 - 3)
                ...
                fle.write("???\n")
                fle.write(
                    repr(self.scans[i].rah).rjust(6) + repr(self.scans[i].ram).rjust(6) + repr(self.scans[i].ras).rjust(
                        6) + repr(self.scans[i].decd).rjust(6) + repr(self.scans[i].decm).rjust(6) + repr(
                        self.scans[i].decs).rjust(6) + "\n")
                fle.write(repr(self.source_ld).rjust(6) + repr(self.source_lm).rjust(6) + repr(self.source_bd).rjust(
                    6) + repr(self.source_bm).rjust(6) + "\n")
                fle.write(
                    repr(self.scans[i].azd).rjust(6) + repr(self.scans[i].azm).rjust(6) + repr(self.scans[i].eld).rjust(
                        6) + repr(self.scans[i].elm).rjust(6) + "\n")
                fle.write(self.scans[i].datestring.rjust(10) + "\n")
                fle.write(repr(int(self.scans[i].STh)).rjust(6) + repr(int(self.scans[i].STm)).rjust(6) + repr(
                    int(self.scans[i].STs)).rjust(6) + "\n")
                fle.write(repr(round(self.scans[i].tsys[ee] / 1000.0, 3)).rjust(8) + "\n")
                fle.write("0".rjust(6) + "\n")
                fle.write(repr(ee).rjust(6) + "\n")
                fle.write("$$$\n")
                fle.write(
                    repr(len(self.scans[i].spectr_bbc_final[ee])).rjust(12) + repr(int(self.scans[i].bw[ee])).rjust(
                        15) + repr(0.25).rjust(15) + repr(self.scans[i].vlsr[ee]).rjust(11) + repr(
                        self.scans[i].rest[ee]).rjust(18) + "\n")
                fle.write(self.scans[i].sourcename + "\n")
                fle.write("***" + "\n")
                fle.write(repr(int(self.scans[i].UTh)).rjust(8) + repr(int(self.scans[i].UTm)).rjust(8) + repr(
                    int(self.scans[i].UTs)).rjust(8) + repr(int(self.scans[i].INT)).rjust(8) + "\n")
                # --- dane ----
